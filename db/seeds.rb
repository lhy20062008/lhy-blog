...
10.times do |i|
	...
	post = Post.create(title: "post #{i} title", body: "post #{i} body" * 100, user_id: user.id)

	rand(20).times do |j|
		article = Article.create(title: "Article #{j}", body: "Article #{j} body" * 100, user_id: user.id, post_id: post.id)
		rand(20).times do |k|
